...
import BasiClassDAO.LastOpenFolderDAO;
...
public class AutoInsertMeasutingMethods {
	...
	public static void AutoInsertMeasutingStartFrame(UsersWBC loginDlg) {
		...
		JFileChooser chooiser = new JFileChooser(nameLastOpenFolder);
		chooiser.setMultiSelectionEnabled(true);
		chooiser.setCursor(Cursor.getDefaultCursor());
	...
	private static String revoveLastFolder(String nameOpenFolder) {
		int kk = nameOpenFolder.lastIndexOf("\\");
		if (kk > 3) {
			nameOpenFolder = nameOpenFolder.substring(0, kk);
		}
	...
	public static void SaveListReportMeasurClassToDBase(ActionIcone round,
	...
	public static void roundWithText(ActionIcone round, String newText, int count) {
...
//					ActionIcone round = new ActionIcone();
					...
					final Thread thread = new Thread(new Runnable() {
						@Override
						public void run() {
							if (checkEmptryPostaplenieField(countMeasur, round2)) {
								if (checkEmptryDozeField(countMeasur, round2)) {
									List<ReportMeasurClass> listReportMeasurClassToSave = AutoInsertMeasutingFrame
											.generateListReportMeasurClassForSaveData(countMeasur,
													listReportMeasurClass);
									AutoInsertMeasutingMethods.SaveMesuring(listReportMeasurClassToSave, round2);
				...
				for (int k = 0; k < 20; k++) {
					if (comboBox_Nuclide[i][k] != null) {
						...
						if (comboBoxTypeMeasur[i].getSelectedItem().toString().equals("M")) {
							...
							postaplenieStr = textField_Postaplenie[i][k].getText();

							if (postaplenieStr.isEmpty() || !(Double
									.parseDouble(postaplenieStr.replaceAll(" ", "").replaceAll(",", ".")) > 0)) {
